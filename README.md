...
Debug endpoint for Alpha Vantage. Returns detailed response including raw API data, parsed values, and any error messages.

Query Parameters:
- `symbol` (string): Single symbol or comma-separated list (e.g., `SPY` or `SPY,QQQ,GLD`)
- If omitted, tests first 5 supported symbols
...
```json
{
  ...
  "results": {
    "SPY": {
      "httpStatus": 200,
      "symbol": "SPY",
      "parsed": { "price": 550.23, "previousClose": 548.50, "tradingDay": "2025-01-03" },
      "isRateLimited": false,
      "isError": false
    }
  }
...
Debug endpoint for Stooq (VIX data). Returns symbol alternatives tried and raw CSV responses.

Query Parameters:
- `symbol` (string): Single symbol or comma-separated list (e.g., `VIX` or `VIX,VVIX`)
- If omitted, tests all supported VIX symbols

## How to Verify Providers

After deployment, test each data source:

### 1. Test Alpha Vantage (ETFs/Equities)

```bash
curl "https://your-domain.com/api/debug/av?symbol=SPY"
```

Expected: `parsed.price` should be a number, `isError: false`, `isRateLimited: false`

If failing:
- Check `ALPHAVANTAGE_API_KEY` is set correctly
- Check `rawResponse` for error messages
- Free tier allows 25 requests/day total

### 2. Test FRED (Treasury Yields)

```bash
curl "https://your-domain.com/api/market" | jq '.capabilities.TNX'
```

Expected: `ok: true`, `sourceUsed: "FRED"`
...
```bash
curl "https://your-domain.com/api/debug/stooq?symbol=VIX"
```

Expected: `success: true`, `data.price` should be a number

If failing:
- Stooq is free but VIX data availability varies
- Dashboard will still function without VIX data

### 4. Full System Test

```bash
curl "https://your-domain.com/api/market" | jq '.summary'
```

Check:
- `capabilities`: Most symbols should have `ok: true`
- `indicators`: Should have `price` values
- `scores`: Should be 0-100 (not all 50s)
- `sectors`: Should have varied scores (not all 50s)
