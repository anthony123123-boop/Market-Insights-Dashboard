...
let serverCache: CacheEntry<MarketDataResponse> | null = null;
let fredCache: CacheEntry<{ indicators: Record<string, Indicator>; latestEpoch: number }> | null = null;
let intradayCache: CacheEntry<{ indicators: Record<string, Indicator>; latestEpoch: number }> | null = null;
const CACHE_TTL_MS = 60 * 60 * 1000; // 1 hour
const FRED_TTL_MS = 20 * 60 * 1000; // 20 minutes
const INTRADAY_TTL_MS = 60 * 1000; // 60 seconds
...
const SCORING_VERSION = 'v2-2026-01-20';
const SCORING_FLAGS = {
  enableEnhancedTrend: process.env['ENABLE_ENHANCED_TREND'] === 'true',
  enableBreadthProxy: process.env['ENABLE_BREADTH_PROXY'] === 'true',
  enableSectorProxyMode: process.env['ENABLE_SECTOR_PROXY_MODE'] === 'true',
  enableSanityTests: process.env.NODE_ENV === 'test' || process.env['ENABLE_SANITY_TESTS'] === 'true',
};
...
const INTRADAY_SYMBOLS: Record<string, { symbol: string; displayName: string }> = {
  SP500: { symbol: 'SPY', displayName: 'S&P 500' },
  NASDAQCOM: { symbol: 'QQQ', displayName: 'Nasdaq Composite' },
  DJIA: { symbol: 'DIA', displayName: 'Dow Jones' },
  VIX: { symbol: 'VIX', displayName: 'VIX' },
};
...
// Proxy-mode mapping (uses existing indicator changePct values, no new API calls)
const SECTOR_PROXY_MODE_RULES: Record<string, { name: string; source: string; min: number; max: number }> = {
  XLK: { name: 'Technology', source: 'QQQ', min: -2.5, max: 2.5 },
  XLF: { name: 'Financials', source: 'SPY', min: -2, max: 2 },
  XLI: { name: 'Industrials', source: 'SPY', min: -2, max: 2 },
  XLE: { name: 'Energy', source: 'OIL', min: -3.5, max: 3.5 },
  XLV: { name: 'Health Care', source: 'SPY', min: -1.5, max: 1.5 },
  XLP: { name: 'Cons. Staples', source: 'SPY', min: -1.5, max: 1.5 },
  XLU: { name: 'Utilities', source: 'SPY', min: -1.25, max: 1.25 },
  XLRE: { name: 'Real Estate', source: 'SPY', min: -2, max: 2 },
  XLY: { name: 'Cons. Disc.', source: 'QQQ', min: -2.5, max: 2.5 },
  XLC: { name: 'Communication', source: 'QQQ', min: -2.5, max: 2.5 },
};
...
function formatNYTimestamp(date: Date): string {
...
function getEpochFromDateString(dateString: string): number {
  return new Date(`${dateString}T00:00:00Z`).getTime();
}

function getCache<T>(cache: CacheEntry<T> | null, ttlMs: number, bypass: boolean): T | null {
  if (bypass || !cache) return null;
...
async function fetchFredSeries(seriesId: string, apiKey: string): Promise<{
  ...
  try {
    ...
    const asOfEpoch = getEpochFromDateString(validObs[0]!.date);
    return { price, previousClose, asOfEpoch };
  } catch {
    return null;
  }
}
...
async function fetchFredIndicators(
  fredKey: string,
  bypassCache: boolean
): Promise<{ indicators: Record<string, Indicator>; warnings: string[]; latestEpoch: number }> {
  const cached = getCache(fredCache, FRED_TTL_MS, bypassCache);
  if (cached) {
    return { indicators: { ...cached.indicators }, warnings: [], latestEpoch: cached.latestEpoch };
  }
  ...
  await Promise.all(
    Object.entries(FRED_SERIES).map(async ([ticker, config]) => {
      ...
      if (result) {
        ...
        const asOfET = formatNYTimestamp(new Date(result.asOfEpoch));
        latestEpoch = Math.max(latestEpoch, result.asOfEpoch);
        indicators[ticker] = {
        ...
        console.log(`[FRED] ${ticker} as of ${asOfET}`);
      } else {
  ...
  if (Object.keys(indicators).length > 0) {
    fredCache = { data: { indicators, latestEpoch }, timestamp: Date.now() };
  } else if (fredCache) {
    warnings.push('FRED fetch failed, using cached data');
    return { indicators: { ...fredCache.data.indicators }, warnings, latestEpoch: fredCache.data.latestEpoch };
  }

  return { indicators, warnings, latestEpoch };
}

async function fetchIntradayIndicators(
  finnhubKey: string | undefined,
  bypassCache: boolean
): Promise<{ indicators: Record<string, Indicator>; warnings: string[]; latestEpoch: number }> {
  if (!finnhubKey) {
    return { indicators: {}, warnings: ['Intraday source unavailable'], latestEpoch: 0 };
  }

  const cached = getCache(intradayCache, INTRADAY_TTL_MS, bypassCache);
  if (cached) {
    return { indicators: { ...cached.indicators }, warnings: [], latestEpoch: cached.latestEpoch };
  }
  ...
  await Promise.all(
    Object.entries(INTRADAY_SYMBOLS).map(async ([ticker, mapping]) => {
      try {
        const url = `${FINNHUB_BASE_URL}?symbol=${encodeURIComponent(mapping.symbol)}&token=${finnhubKey}`;
        const response = await fetchWithRetry(url);
        if (!response || !response.ok) {
          warnings.push(`Intraday: ${ticker}`);
          return;
        }
        const data = await response.json() as { c: number; pc: number; d: number; dp: number; t: number };
        if (!data || typeof data.c !== 'number' || data.c <= 0) {
          warnings.push(`Intraday: ${ticker}`);
          return;
        }
        const previousClose = typeof data.pc === 'number' ? data.pc : data.c;
        const change = typeof data.d === 'number' ? data.d : data.c - previousClose;
        const changePct = typeof data.dp === 'number'
          ? data.dp
          : previousClose !== 0 ? ((data.c - previousClose) / previousClose) * 100 : 0;
        const asOfEpoch = typeof data.t === 'number' && data.t > 0 ? data.t * 1000 : Date.now();
        const asOfET = formatNYTimestamp(new Date(asOfEpoch));
        latestEpoch = Math.max(latestEpoch, asOfEpoch);
        indicators[ticker] = {
        ...
        console.log(`[INTRADAY] ${ticker} via ${mapping.symbol} as of ${asOfET}`);
      } catch (error) {
        console.warn(`Intraday fetch failed for ${ticker}:`, error);
        warnings.push(`Intraday: ${ticker}`);
      }
    })
  );
  ...
  if (Object.keys(indicators).length > 0) {
    intradayCache = { data: { indicators, latestEpoch }, timestamp: Date.now() };
  }
...
// Main data fetch
async function fetchAllData(fredKey: string, bypassCache: boolean): Promise<{
  indicators: Record<string, Indicator>;
  warnings: string[];
  sectors: Sector[];
  latestEpoch: number;
  ...
  const indicators: Record<string, Indicator> = {};
  const warnings: string[] = [];
  const { indicators: fredIndicators, warnings: fredWarnings, latestEpoch: fredLatestEpoch } =
    await fetchFredIndicators(fredKey, bypassCache);
  Object.assign(indicators, fredIndicators);
  warnings.push(...fredWarnings);

  const finnhubKey = process.env['FINNHUB_API_KEY'];
  const { indicators: intradayIndicators, warnings: intradayWarnings, latestEpoch: intradayLatestEpoch } =
    await fetchIntradayIndicators(finnhubKey, bypassCache);
  Object.assign(indicators, intradayIndicators);
  warnings.push(...intradayWarnings);

  const latestEpoch = Math.max(fredLatestEpoch, intradayLatestEpoch);
  ...
  const sectors = buildSectorScores(indicators, warnings, SCORING_FLAGS);
  ...
  return {
    ...
    sectors,
    latestEpoch,
    hadIntraday: Object.keys(intradayIndicators).length > 0,
  };
}
...
async function safeFetchAllData(
  ...
  const timeoutFallback = new Promise<{
    ...
    setTimeout(() => {
      ...
      resolve({
        ...
        sectors: buildSectorScores(null, timeoutWarnings, SCORING_FLAGS),
        latestEpoch: 0,
        hadIntraday: false,
      });
  ...
  try {
    return await Promise.race([fetchAllData(fredKey, bypassCache), timeoutFallback]);
  } catch {
    const warnings = ['Data fetch failed'];
    return {
      indicators: {},
      warnings,
      sectors: buildSectorScores(null, warnings, SCORING_FLAGS),
      latestEpoch: 0,
      hadIntraday: false,
    };
  }
...
function calculateScores(
  indicators: Record<string, Indicator>,
  warnings: string[],
  flags: typeof SCORING_FLAGS
): {
  scores: { short: number; medium: number; long: number };
  categoryScores: Record<string, { score: number; available: boolean; weight: number }>;
  scoringWeights: MarketDataResponse['scoringWeights'];
} {
  ...
  // Breadth (small/large ratio + optional proxy blends)
  let breadthScore = 50, breadthAvailable = false;
  const smallLarge = indicators['SMALL_LARGE'];
  try {
    if (flags.enableBreadthProxy) {
      const breadthInputs: Array<{ value: number; min: number; max: number }> = [];
      if (smallLarge?.changePct !== undefined) {
        breadthInputs.push({ value: smallLarge.changePct, min: -1, max: 1 });
      }
      const rspSpy = indicators['RSP_SPY_RATIO'];
      if (rspSpy?.changePct !== undefined) {
        breadthInputs.push({ value: rspSpy.changePct, min: -0.7, max: 0.7 });
      }
      const iwmSpy = indicators['IWM_SPY_RATIO'];
      if (iwmSpy?.changePct !== undefined) {
        breadthInputs.push({ value: iwmSpy.changePct, min: -0.7, max: 0.7 });
      }

      if (breadthInputs.length > 0) {
        const total = breadthInputs.reduce((sum, input) => sum + normalize(input.value, input.min, input.max), 0);
        breadthScore = total / breadthInputs.length;
        breadthAvailable = true;
      } else {
        warnings.push('Breadth proxy enabled but data unavailable');
      }
    } else if (smallLarge?.changePct !== undefined) {
      breadthScore = normalize(smallLarge.changePct, -1, 1);
      breadthAvailable = true;
    }
  } catch (error) {
    console.error('Breadth proxy calculation failed:', error);
    warnings.push('Breadth proxy calculation failed');
  }
  ...
  const shortTrendWeight = flags.enableEnhancedTrend ? 0.5 : 0.4;
  const shortBaseWeight = 1 - shortTrendWeight;
  const scoringWeights: MarketDataResponse['scoringWeights'] = {
    category: {
      trend: categoryScores['trend']?.weight ?? 0,
      volTail: categoryScores['volTail']?.weight ?? 0,
      creditLiquidity: categoryScores['creditLiquidity']?.weight ?? 0,
      rates: categoryScores['rates']?.weight ?? 0,
      breadth: categoryScores['breadth']?.weight ?? 0,
      usdFx: categoryScores['usdFx']?.weight ?? 0,
    },
    composite: {
      short: { base: shortBaseWeight, trend: shortTrendWeight },
      medium: { base: 1 },
      long: { base: 0.5, credit: 0.3, rates: 0.2 },
    },
  };

  return {
    scores: {
      short: Math.round(Math.max(0, Math.min(100, baseScore * shortBaseWeight + trendScore * shortTrendWeight))),
...
function buildSectorScores(
  ...
  try {
    const rules = flags.enableSectorProxyMode ? SECTOR_PROXY_MODE_RULES : SECTOR_PROXY_RULES;
    const sectors = Object.entries(rules).map(([ticker, rule]) => {
      const indicator = indicators ? indicators[rule.source] : undefined;
      const hasPrice = indicator && indicator.price !== undefined;
      const hasChange = indicator && indicator.changePct !== undefined;

      if (!indicator || (!hasPrice && !hasChange)) {
        return {
          ticker,
          name: rule.name,
          score: flags.enableSectorProxyMode ? null : 50,
          available: false,
          proxy: rule.source,
        };
      }
      ...
      let scoreBase = 50;
      if ('mode' in rule) {
        scoreBase = rule.mode === 'pct'
          ? normalize(indicator.changePct ?? 0, rule.min, rule.max, rule.invert)
          : normalize(indicator.price ?? 0, rule.min, rule.max, rule.invert);
      } else {
        scoreBase = normalize(indicator.changePct ?? 0, rule.min, rule.max);
      }
    ...
    console.error('Sector scoring failed:', error);
    warnings.push('Sector scoring failed');
    return Object.entries(SECTOR_PROXY_RULES).map(([ticker, rule]) => ({
      ticker,
      name: rule.name,
      score: flags.enableSectorProxyMode ? null : 50,
      available: false,
...
function runSanityTests(
  ...
  const allScores = [scores.short, scores.medium, scores.long];
  const hasInvalid = allScores.some(score => Number.isNaN(score) || score < 0 || score > 100);
  if (hasInvalid) {
    warnings.push('Sanity test failed: score out of range');
  }

  const invalidCategory = Object.values(categoryScores).some((cat) => Number.isNaN(cat.score) || cat.score < 0 || cat.score > 100);
  if (invalidCategory) {
    warnings.push('Sanity test failed: category score out of range');
  }

  const invalidSector = sectors.some((sector) => typeof sector.score === 'number' && (Number.isNaN(sector.score) || sector.score < 0 || sector.score > 100));
  if (invalidSector) {
    warnings.push('Sanity test failed: sector score out of range');
  }
}

// Main handler
export async function handler(event: { httpMethod: string; queryStringParameters?: Record<string, string> }): Promise<{
  statusCode: number;
  ...
  const bypassCache = event.queryStringParameters?.nocache === '1';
  ...
  try {
    ...
    const { indicators, warnings, sectors, latestEpoch, hadIntraday } = await safeFetchAllData(fredKey, bypassCache);
    if (Object.keys(indicators).length === 0 && serverCache && !bypassCache) {
      return {
        statusCode: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    const updatedAtNY = latestEpoch > 0 ? formatNYTimestamp(new Date(latestEpoch)) : getNYTimestamp();
    const dataStatus: 'LIVE' | 'EOD (FRED)' | 'Delayed' = hadIntraday
      ? 'LIVE'
      : Object.keys(indicators).length > 0
        ? 'EOD (FRED)'
        : 'Delayed';
    let scores = { short: 50, medium: 50, long: 50 };
    let categoryScores: Record<string, { score: number; available: boolean; weight: number }> = {};
    let scoringWeights: MarketDataResponse['scoringWeights'] = {
      category: {},
      composite: {
        short: { base: 0.6, trend: 0.4 },
        medium: { base: 1 },
        long: { base: 0.5, credit: 0.3, rates: 0.2 },
      },
    };
    try {
      ({ scores, categoryScores, scoringWeights } = calculateScores(indicators, warnings, SCORING_FLAGS));
    } catch (error) {
      console.error('Score calculation failed:', error);
      warnings.push('Score calculation failed');
    }

    const status = generateStatus(scores, categoryScores, indicators);
    if (SCORING_FLAGS.enableSanityTests) {
      try {
        runSanityTests(scores, categoryScores, sectors, warnings);
      } catch (error) {
        console.error('Sanity tests failed:', error);
        warnings.push('Sanity tests failed');
      }
    }
