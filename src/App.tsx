...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f12] text-white">
      {/* Background gradient */}
      <div className="fixed inset-0 bg-gradient-to-br from-emerald-950/20 via-neutral-950/20 to-neutral-950/60 pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
        ...
        <header className="mb-6 flex flex-col gap-4">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="flex flex-col gap-2">
              <h1 className="text-4xl md:text-6xl font-black tracking-tight text-white text-left">
                MARKET INSIGHTS DASHBOARD
              </h1>
              <div className="flex flex-col gap-1 text-xs text-slate-300">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-slate-400">Last updated:</span>
                  <span className="text-slate-100">{data.updatedAtNY}</span>
                  {isStale && (
                    <span className="text-amber-400/80 text-[10px]">(cached)</span>
                  )}
                  <span className="text-slate-500">•</span>
                  <span className="text-slate-400">Scoring:</span>
                  <span className="text-slate-100">{data.scoringVersion ?? '—'}</span>
                  <span className="text-slate-500">•</span>
                  <span className="text-slate-400">Status:</span>
                  <span className="text-slate-100">{data.dataStatus ?? '—'}</span>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-slate-400">Current time:</span>
                  <span className="text-slate-100">{data.currentTimeNY}</span>
                </div>
              </div>
            </div>
            <div className="flex items-start justify-end gap-2">
              <button
                type="button"
                onClick={refetch}
                className="rounded-full border border-white/10 bg-white/5 p-1.5 text-slate-300 transition hover:border-emerald-400/60 hover:text-white"
        ...
        <div className="grid grid-cols-4 gap-4 mb-6" style={{ gridTemplateRows: '160px auto' }}>
          {/* Short Term - Row 1, Col 1 */}
          <FrostedCard className="flex items-center justify-center px-3 py-2" noPadding>
            <ScoreGauge
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {displayIndicators.map((indicator) => (
              <IndicatorPill key={indicator.ticker} indicator={indicator} />
            ))}
          </div>
        </section>
